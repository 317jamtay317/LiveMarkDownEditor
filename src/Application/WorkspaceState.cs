...
/// order, the Recent Files paths (newest first), the open Folder Workspace's root path, and the Panel
/// Layout. Only saved documents are recorded — a Tab with no Watched File, and any unsaved edits, are
/// never persisted (INV-037); the Folder Tree itself is never persisted, only its root, which is
/// re-enumerated on Restore (INV-045).
/// </summary>
